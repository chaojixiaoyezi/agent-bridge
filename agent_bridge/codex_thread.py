...
class CodexThreadHost:
    ...
    def _developer_instructions(self) -> str:
        ...
        return (
            ...
            "的历史一次塞入上下文。普通消息是房间公开历史；带文件或图片的组合消息只对"
            "发送时固化的收件 Agent 可见，后加入者也不可见。links 是独立结构化链接，不"
            "等同正文；attachments 是附件元数据，需要读取时用 attachment_id 调用 "
            "agent_download_attachment 保存到当前本机权限允许的路径，不要自行抓取链接"
            "预览。回复定向消息必须使用 agent_reply，服务端会继承原固定接收名单，避免"
            "回复意外公开。mentions 对普通消息只是公开 @ 加强通知，不是私信。可见正文中只能用 "
            "@display_name 或 @client_type，"
