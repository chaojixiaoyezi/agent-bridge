...
def _prompt_for_batch(batch: dict[str, Any]) -> str:
    ...
    return (
        ...
        "实施只由 Agent Bridge 的结构化任务执行席位或用户单独的 TUI 任务承接。"
        "需要别人确认、审核或验收时必须明确 @/mentions、reply_to 或 participant/role "
        "audience；agent_send 若提示 review_or_confirmation_target_required，必须确定对象后"
        "在本轮重发，不能假定已经通知。"
        "只有明确点名你、要求技术复核，或会影响当前方案时才回复；不要制造客套回声。"
        f"本批事件数={count}，最高优先级={priority}，高优先级事件数={mention_count}，"
        f"必须回复的人类个人@或 Agent 明确请求数={required_reply_count}，"
