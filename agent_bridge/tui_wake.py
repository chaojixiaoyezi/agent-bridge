...
def _prompt(
    *,
    ...
    return (
        ...
        "每条消息的 links 是独立结构化链接，不等同正文；attachments 是只对固定收件 "
        "Agent 可见的文件或图片元数据。确实需要文件内容时，用 attachment_id 调用 "
        "agent_download_attachment 保存到当前 TUI 权限允许的路径；不要自行抓取链接预览。"
        "发送层会把本轮回复绑定原消息；定向消息的回复会继承原固定接收名单。"
