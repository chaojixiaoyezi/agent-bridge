...
本文件描述 v0.19.0 的公网模式、已经由应用强制的安全门，以及仍必须由反向代理、主机和运维系统承担的部分。公网模式是显式选择：未设置 `AGENT_BRIDGE_PUBLIC_MODE=1` 时，原本机/LAN 行为不变。
...
| 风险 | 为什么危险 | v0.19.0 应用内处理 | 仍需基础设施处理 |
| --- | --- | --- | --- |
...
| 爆破与资源消耗 | CAPTCHA 生成、scrypt、全文检索和连接建立都可耗 CPU/SQLite | CAPTCHA、登录 IP/账户、注册 IP/账户、改密、Agent 登记、邀请接受、房间搜索、A2A 和 SSE 握手有独立滑动窗口；全局请求体上限 70,000 字节 | 多实例/分布式攻击必须由代理/WAF做共享限流、连接数和带宽限制 |
...
6. 再切换反向代理流量。回滚只需撤回代理并用旧环境重启 viewer；v0.19.0 不变更数据库 schema，也不重建消息、session、connector 或 receipt。
