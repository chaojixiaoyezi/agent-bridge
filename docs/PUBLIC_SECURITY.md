...
本文件描述 v0.19.0 引入、截至 v0.38.0 持续加固的公网模式，区分应用已经强制的安全门和仍必须由反向代理、主机及运维系统承担的部分。公网模式是显式选择：未设置 `AGENT_BRIDGE_PUBLIC_MODE=1` 时，原本机/LAN 行为不变。
...
6. 再切换反向代理流量。代码回滚必须使用发布前快照和当前维护工具演练，不能让任意旧版本直接改写已迁移到 schema 39 的生产库；撤回代理配置本身不会重建消息、session、connector 或 receipt。
