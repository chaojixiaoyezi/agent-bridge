...
- 创建房间时可显式选择实验性的“整合聊天室”。它只允许一个稳定 Agent 身份，输入区固定为结构化 TUI 任务；当前 Claude Code 适配会复用该房间的持久 `stream-json` session，并把可见模型文本、工具开始/完成/失败、权限等待和回合结果按顺序显示在 Web 中。它不保存隐藏思考或原始事件，也不会把投影失败变成任务失败；普通聊天室仍使用原来的聊天与任务路径。
...
消息上限、1260 条历史/最多 96 个消息 DOM 的虚拟列表、滚动锚定、左右面板折叠、主题切换和 390px 窄屏布局回归，并输出
