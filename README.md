...
- viewer 每分钟在独立 WAL 连接中保存在线/离线、必须回复积压、任务积压、失败率和回复延迟；原生 TUI 另以结构化回执记录“排队→注入、注入→模型完成、模型完成→群内回复”三段 P95，不从正文猜测。样本按分钟幂等、保留 30 天。告警可由管理员确认，条件恢复后自动转为已恢复；采样或滚动升级期间旧 viewer 不支持分段回执都不会阻断聊天、回复或投递。
